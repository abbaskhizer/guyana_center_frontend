          ...
          Expanded(
            child: SizedBox(
              ...
              child: TextFormField(
                ...
                onChanged: (v) => controller.searchText.value = v.trim(),
...
class _BrowseFilterSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final isLanding = controller.isLandingState;
      final query = controller.displayQuery(web);

      if (isEmptyState || isLanding) return const SizedBox.shrink();
...
class _SortMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        _buildItem(context, 'Newest', Icons.access_time_rounded),
        _buildItem(context, 'Price: Low to High', Icons.arrow_downward_rounded),
        _buildItem(context, 'Price: High to Low', Icons.arrow_upward_rounded),
      ],
      child: Container(
  ...
  PopupMenuItem<String> _buildItem(
    ...
    final cs = Theme.of(context).colorScheme;
    final active = controller.sortLabel.value == value;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Obx(() {
        ...
        if (controller.isLandingState) {
          return Container(
            width: double.infinity,
            color: cs.surface,
            child: _LandingSearchState(isWeb: isWeb),
          );
        }
...
class _LandingSearchState extends StatelessWidget {
  final bool isWeb;

  const _LandingSearchState({required this.isWeb});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            'Search for vehicles, real estate, jobs,\nand much more across Guyana.',
            textAlign: TextAlign.center,
            style: theme.textTheme.bodyMedium?.copyWith(
              color: cs.onSurface.withOpacity(0.6),
              height: 1.5,
          ...
          Wrap(
            ...
            children: controller.popularSearches.map((item) {
              return InkWell(
                onTap: () {
                  controller.setSearch(item);
                },
                borderRadius: BorderRadius.circular(999),
                child: _PopularSearchChip(label: item, web: isWeb),
              );
            }).toList(),
          ),
